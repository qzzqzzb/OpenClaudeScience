...
```text
1. 先做一轮人工回归，确认第一至第四阶段没有破坏现有功能
2. 再评估是否需要开发模式 mock runtime 开关
3. 梳理当前主链路和旁路 provider 的差距
4. OpenCode 进入下一轮 adapter 可行性评估，不在第四阶段继续扩大范围
```

## 第五阶段：测试、文档、回归验证（当前架构改造范围已完成）
...
```text
docs/architecture/第五阶段回归验证.md
docs/architecture/架构改造总验收.md
```
...
当前范围已完成：
- 前端 API client 收口
- 后端 route -> service -> adapter 分层
- shared adapter contract
- Agent Runtime Adapter 边界
- Runtime Protocol + Mock/LangGraph 旁路验证
- 主链路 runtime facade 已暴露 protocol event 通道
- 主链路 submit/stop 已收口到 ClientAgentRuntimeAdapter
- Runtime provider registry 已补齐
- Runtime provider settings 解析已补齐
- 自动化回归与验收文档

仍属于后续产品/底层替换决策：
- 主聊天链路是否切到中立 provider contract
- OpenCode 是作为 runtime provider 还是 tool adapter 接入
- 是否新增 runtime provider 配置入口
- 是否新增开发模式 mock runtime 开关
```
