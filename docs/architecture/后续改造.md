...
```text
1. 继续补 LangGraph raw event -> AgentRuntimeRunEvent 映射测试
2. 设计真实 adapter 如何组合 lifecycle helper + stream mapper
3. 暂不挂主链路，先做旁路转换器或测试
...
3. 第四阶段已完成第一轮旁路验证：Runtime Protocol、MockRuntimeProvider、LangGraph event mapper、lifecycle helper
```
