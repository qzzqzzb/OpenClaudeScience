...
会话列表、归档、恢复、assistant resolve 已开始走 Agent Runtime Adapter。
...
- 它是 Agent 主链路核心。
- 先没有 contract 就拆，会把底层协议和业务规则搅在一起。
- 等 route/service/adapter 边界稳定后再动更安全。

## 第四阶段：测试、文档、回归验证（贯穿所有阶段）
...
```text
1. 第三阶段 3.1：Agent Runtime Adapter 入口已启动
2. 第三阶段 3.2：thread 查询、state/history、pending preview、metadata/state update 已迁移到 runtime adapter
3. 第三阶段 3.3：useChat.ts 的 useStream 入口已收口到 useAgentRuntimeStream，下一步定义 run stream / stop / retry / interrupt 协议
```
