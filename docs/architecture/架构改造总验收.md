...
本轮后续清理已经移除 `ui/src/app/api/**/_lib` 中的 TypeScript 底层实现：

```text
workspace 文件系统 / Office 预览 / 桌面打开 -> workspace domain adapters
runtime 进程状态与重启 -> runtime domain adapters
remote SSH / runtime / backend CLI -> remote domain adapters + shared SSH adapter
compute 认证 / 存储 / 作业协议 -> compute domain adapters
skills 配置 / 导入 / frontmatter -> skills domain adapters
update 检查 / 安装 / 回滚 -> update domain adapters
本地目录选择 -> shared platform adapter
```

API route 现在只负责 HTTP 边界；domain service 和 concrete adapter 不再反向依赖 API route `_lib`。结构守卫测试会扫描生产 TypeScript，阻止旧依赖重新进入。
...
node --test --experimental-strip-types tests/*.test.mts -> 71 passed
...
Next.js 完成编译、TypeScript、页面数据收集、34 个静态页面生成和 build trace 收集。
