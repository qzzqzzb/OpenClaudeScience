...
以下不是当前架构改造未完成，而是后续产品/平台化决策：

```text
主聊天链路真正切到 AgentRuntimeProtocolProvider
OpenCodeRuntimeAdapter 真实接入
DeepAgents 替换
agent_graph.py 重写
runtime provider 配置入口
...
node --test --experimental-strip-types tests/*.test.mts -> 62 passed
...
```text
是，当前范围已完成，后续替换 runtime / OpenCode 属于下一轮平台化演进。
```
